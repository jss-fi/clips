...
Use the development line followed by `-nightly` for a nightly, or the development line alone for an explicitly approved stable promotion (for example, `npm run release -- 0.7`). The command refuses to run outside the primary `main` worktree or with uncommitted changes. It checks the expected version line and credentials, runs the full test suite, creates and commits release metadata, selects a fresh runtime build when runtime-affecting files changed, builds the artifacts, publishes the correct channel or channels, and waits for R2, CDN, and GitHub verification. Stable promotions always build the full setup installer, even when the existing media runtime can be reused. Safe retention cleanup is attempted afterward, but an unrelated older archive that cannot yet be pruned is retained and reported without invalidating the new release. Pass `--fresh` to force runtime restaging when preparing new artifacts. If a prepared release already has a checksum-verified artifact manifest, `--fresh` fails closed because rebuilding that version could conflict with immutable assets; retry without the flag or prepare a new release/version instead.

If metadata cannot be committed, the workflow restores the original version files and index automatically. If a later build, upload, or verification step fails, fix the reported problem and run the identical command again. Completed builds have an ignored local manifest containing the size and SHA-512 of every payload and metadata file. A prepared retry verifies that manifest and the signed update metadata, then reuses the exact original artifacts without rebuilding them; missing or modified same-version artifacts stop the retry before publication. The workflow never pushes branch commits; publishing pushes only the immutable release tag after the public update is live.
